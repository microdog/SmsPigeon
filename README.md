...
| 慢心跳（2 秒周期短亮） | 已注册但固件未初始化——等待 `AT+INIT` |
| 常亮 | 已初始化且网络已注册——正常工作中 |
| 三连闪 | 收到一条短信（活动指示） |
...
1. **先看日志有没有"收到短信"**：模块收到任何短信都会先打印
   `I/user.sp_forward 收到短信 <号码> <内容>`（内核层还有
   `CMI_SMS_NEW_MSG_IND` / `D/sms` 行）。
   - 有这行但命令无应答 → 命令格式问题：半角符号、IMEI 是否与日志
     `self_info` 行一致（见 docs/commands.md）；
   - 完全没有这行 → 短信根本没到模块，继续往下。
