...
（10 个测试文件 514 项断言）。HTTP 通道的真实推送与硬件行为（短信收发、
...
本项目（SmsPigeon 短信鸽）是面向个人合法用途的开源学习项目，按"现状"
提供。作者不对本项目作任何明示或默示的担保（包括适销性、特定用途
...
- 本项目**仅供学习、研究与个人合法通信用途**。严禁将本项目或其衍生
  形态用于：短信验证码接码/收码平台、电信网络诈骗、绕过手机卡实名
  制度、群发垃圾短信或骚扰信息，或任何违反中国法律及使用者所在地
  法律、法规的活动；
- 使用本项目产生的一切行为及其后果，均由使用者本人承担。作者开源
  本项目不构成对任何违法用途的授意、教唆或帮助，与使用者的行为
  无共同故意；
- **SIM/物联网卡用途合规由使用者自行确认与承担**：运营商物联网卡
  合同普遍禁止"人卡分离""改变用途"（如用于个人通信/短信转发）。
  由此可能产生的合同违约后果（包括但不限于停卡、销号、追缴费用）
  由使用者自行承担，与作者无关。部署前请阅读你的运营商合同条款
  并确认卡片用途限制；
- 本项目不销售、不附带、不代购任何 SIM/物联网卡，不提供代部署、
  托管或任何收费服务；与上述相关的功能请求与 issue 将被拒绝并关闭；
- 部分国家或地区对短信网关类设备/软件有额外监管要求，使用者在
  部署前应自行确认并遵守当地规定；
