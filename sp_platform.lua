...
-- 核心发送（调用方已持锁）。
-- 已知限制：SMS_SENT 是全局广播且 sms.send 的同步返回不携带
-- msg_ref，事件无法与本条提交精确关联；发送互斥已把串扰窗口压缩到
-- "相邻两条、同一时刻仅一个等待者"，上一条超时（10s 无事件）放行后
-- 的迟到事件理论上会被下一条错认。影响仅限日志与应答文案误报：
-- 超时路径保守按"已提交"处理，不产生安全后果；任何基于序号/时间的
-- 迟到事件吞咽启发式都会把真实事件误吞（把成功误报为超时），收益
-- 为负，故不做。若后续内核 sms.send 返回 msg_ref，可改为精确关联。
