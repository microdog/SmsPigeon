...
-- Q1 防锁死：白名单关闭且删空后，开启白名单必须被拒绝
send(ME, "信鸽，关闭白名单")
send(ME, "信鸽，删除白名单，13800138000")
eq(#sp_config.get().whitelist, 0, "白名单关闭时允许删空")
is_cmd, reply = send(ME, "信鸽，开启白名单")
contains(reply, "ERROR", "空白名单开启被拒绝(防锁死)")
contains(reply, "增加白名单", "拒绝时提示补号码")
send(ME, "信鸽，增加白名单，13800138000")
send(ME, "信鸽，开启白名单")
eq(sp_config.get().wl_on, true, "补号码后可正常开启")
