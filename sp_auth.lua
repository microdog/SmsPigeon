...
1. 初始化门禁：固件未初始化时，只放行 AT+INIT 命令，其余静默丢弃；
2. 白名单门禁：白名单开启时，发送者号码必须在白名单内；
3. 密码门禁：设置密码后，命令必须以密码为前缀（由 sp_at.parse 强制，
   密码模式下 AT 前缀不再有效）。

白名单与密码可独立开/关；两者同时关闭时任何 AT 命令均可控制本机，
命令层（sp_commands）会在这种状态下给出风险提示。

号码归一化：仅保留数字；长度超过 11 位时取末 11 位（处理 +86 前缀），
因此白名单中只需存储 11 位裸号码。本固件面向中国大陆场景。
]]

local sp_auth = {}

--[[
归一化手机号：剔除非数字字符，超过 11 位取末 11 位。
无效输入返回 nil。示例：
